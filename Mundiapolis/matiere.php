			...
			<div class="page-content">
				...
				<div class="row clearfix">
					...
					<div class="grid_12">
						...
						<div>
							<h2 class="tac"><?php echo $matiereDetail["libelle_matiere"]; ?></h2>
                    ...
                    <br> </br>
						<div>
							...
							<hr>
							<?php
							$competencematiere = $model->getCompetencematiere($codeMat);
							if ($competencematiere->num_rows > 0){

								while($resultCompetenceMatiere = $competencematiere->fetch_assoc()){
									 echo $model->getCategorieByCode($resultCompetenceMatiere['id_Categorie']);
								}
							}else{
								?> <li> <?php echo "<span style='color: red;font-size: 15px;'>Pas de Compétence pour cette matière</span>";?> </li><?php
							}
						...
						<div>
							<h3>Compétences pré_requises</h3>
							 <?php
								$PreRequis = $model->getPreRequis($codeMat);

								if ($PreRequis->num_rows>0) {
									while($resultPreRequis = $PreRequis->fetch_assoc()){
										$model->getCategorieByCode($resultPreRequis['id_Categorie']);
									}
								}else{
								?> <li> <?php	echo "<span style='color: red;font-size: 15px;'>Pas de Pré_requis pour cette matière</span>";?></li>
								<?php
								}
							?>
							<hr>
						...
						<div>
							<h3>Matières pré_requises</h3>
						
							<?php
							echo $model->getMatiereByCode($codeMat);
							?>
							<hr>
						</div>
					</div>
